...
def main():
    st.title("Schema Mapping Visualization")
    ...
    if uploaded_file is not None:
        ...
        if len(selected_tables) >= 2:
            ...
            for edge in G.edges():
                source = edge[0].split('\n')
                target = edge[1].split('\n')
                st.write(f"• {source[0]} ({source[2].strip('()')}) → {target[0]} ({target[2].strip('()')})")
                connection_count += 1
            
            if connection_count == 0:
                st.info("No connections found at this threshold. Try lowering the threshold value.")
        else:
