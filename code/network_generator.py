...
def create_embeddings(table_data, table_name, model,numerical=1):
    ...
    if numerical==1:
      for idx, item in enumerate(table_data):
        ...
        #   combined_emb = np.concatenate([emb_category,emb_subcategory,emb_data])  # concatenate the vectors

          embeddings.append((table_name, category, '->'.join(subcategory), idx, combined_emb))
    else:
      for idx, item in enumerate(table_data):
        category, subcategory, data = item[0], item[1:-1], item[-1]
        # print(f"{category}->{subcategory}->{data}")
        emb_category = model.encode(category)
        emb_subcategory=0
        for index,value in enumerate(subcategory):
          emb_subcategory+=model.encode(value)*(len(subcategory)-(index))/len(subcategory)
...
def create_similarity_network(information_to_be_checked, model,numerical,file_name, similarity_threshold):
    ...
    try:
        plt.savefig(f'/home/sa23k/embedding_based_schema_mapping/mappings/{file_name}/{similarity_threshold*100}_threshold.png')
    except:
        os.makedirs(f'/home/sa23k/embedding_based_schema_mapping/mappings/{file_name}',exist_ok=True)
        plt.savefig(f'/home/sa23k/embedding_based_schema_mapping/mappings/{file_name}/{similarity_threshold*100}_threshold.png')
    plt.show()
    return G, pos
